...
class AccountMoveManuallySetCommissionsWizard(models.TransientModel):
    ...
    account_move_partner_id = fields.Many2one(
        related="account_move_id.partner_id", string="Sales Order Customer"
    )
    ...
    def action_save_commissions(self):
        for sol in self.account_move_id.line_ids.filtered(
            lambda x: not x.commission_free
        ):
...
class SaleOrderManuallySetCommissionsWizardAgents(models.TransientModel):
    ...
    manual_commission_wizard_id = fields.Many2one(
        "account.move.manual.commissions.wizard", string="Manual Commission Wizard"
    )

    agent_id = fields.Many2one(
        comodel_name="res.partner", domain="[('agent', '=', True)]", required=True
    )
    commission_id = fields.Many2one(comodel_name="commission", required=True)
